...
class JankScreenAccumulatorTest {
    ...
    fun `frame recorded before any screen is visible is dropped`() {
        registry.recordFrame(isJank = true, frameDurationNanos = 40_000_000L, frameBudgetNanos = 16_666_666L)
        // Nothing to assert on directly, but a later screen shouldn't see it.
        registry.onScreenVisible("Activity#1", isContent = false)
        registry.recordFrame(isJank = false, frameDurationNanos = 8_000_000L, frameBudgetNanos = 16_666_666L)
    ...
    fun `fragment hosted in an activity gets frames instead of the host activity`() {
        ...
        registry.recordFrame(isJank = true, frameDurationNanos = 40_000_000L, frameBudgetNanos = 16_666_666L) // activity only
